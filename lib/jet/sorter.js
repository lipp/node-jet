...
exports.create = function (options, notify) {
  ...
  var sorter = function (notification, initializing) {
    ...
    var wasInRange = checkRange(index)
    if (!wasInRange) {
      if (sort(notification, sorted[0]) > 0 || sort(notification, sorted[sorted.length - 1]) > 0) {
        return
      }
    }
    ...
    var isInRange = checkRange(newIndex)
    ...
    if (wasInRange && isInRange) {
      ...
      if (sorted.length === (to - from + 1)) {
        message.append = sorted[sorted.length - 1]
      }
    } else if (!wasInRange && isInRange) {
